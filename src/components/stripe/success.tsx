...
const STATUS_CONTENT_MAP: Record<PaymentStatus | 'default', StatusContent> = {
  succeeded: {
...
function getStatusContent(status: Stripe.PaymentIntent.Status): StatusContent {
  if (status in STATUS_CONTENT_MAP) {
    return STATUS_CONTENT_MAP[status as PaymentStatus];
  }
  return STATUS_CONTENT_MAP.default;
}
  ...
  try {
    ...
    return (
      <div id="payment-status">
        ...
        <h2 id="status-text">{STATUS_CONTENT_MAP.default.text}</h2>
        <a id="retry-button" href="/">
          Try again
        </a>
      </div>
